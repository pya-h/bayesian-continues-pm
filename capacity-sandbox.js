...
(() => {
  ...
  const activeMod = () =>
    modifierFor(curFix === 'none' || FIXES[curFix].explainOnly ? 'none' : curFix);
    ...
    const status = mod.haircut
      ...
      : g.blocked
        ? {
            cls: 'sell',
            txt: mod.soft
              ? 'AT BACKSTOP — congestion priced, hard floor reached'
              : 'BLOCKED — risk-opening buys rejected (offsetting trades & sells still fill)',
          }
        : {
            cls: 'buy',
            txt: mod.soft ? 'OPEN — soft cap pricing the approach' : 'OPEN — buys admitted',
          };
  ...
  function statCells(st, mod, g) {
    ...
    const cells = [
      ...
      [
        'Fair · ' + specLabel(spec),
        fmt(f, 3) + '  (' + (dF >= 0 ? '+' : '−') + fmt(Math.abs(dF), 3) + ')',
        'accent',
      ],
      ['Pool cash' + (mod.extraCash ? ' (+fund)' : ''), money(g.cashB), ''],
      [
        'Required reserve (' +
          (mod.alpha * 100).toFixed(0) +
          '%)' +
          (mod.reserveScale < 1 ? ' ×hedge' : ''),
        money(g.R),
        'sell',
      ],
      [
        'Utilisation ρ = R∕cash',
        fmt(g.rho, 3) + (mod.haircut ? '' : '  (gate ≤ ' + fmt(1 / g.m, 2) + ')'),
        g.rho > 1 ? 'sell' : g.rho > 1 / Math.max(g.m, 1e-9) ? 'warn' : 'buy',
      ],
  ...
  function payoutBox(st, mod, spec) {
    ...
    const rows = [
      ...
      [
        'Worst-case (' + (mod.alpha * 100).toFixed(0) + '%) / contract',
        fmt(perMax, 3),
        'payoff at the tail outcome',
      ],
    ...
    const sNote = mod.haircut
      ? sFac < 0.999
        ? 'pool is short: winners receive ' + (sFac * 100).toFixed(1) + '% of face'
        : 'fully backed right now — keep buying to push it below 1'
      : 'fully backed — payouts are not contingent';
    body +=
      '<tr class="srow"><td>Solvency factor s (' +
      (mod.alpha * 100).toFixed(0) +
      '% scenario)</td><td class="num ' +
      sCls +
      '">' +
      fmt(sFac, 3) +
      '</td><td class="muted">' +
  ...
  function positionsTable(st, mod, sFac) {
    ...
    st.pos.forEach((p) => {
      ...
      rows +=
        '<tr><td>' +
        (p.trader === 'You' ? '<strong>You</strong>' : p.trader) +
        '</td>' +
        '<td class="cap-ct">' +
        specLabel(p.spec) +
        '</td>' +
        ...
        '">' +
        (pnl >= 0 ? '+' : '−') +
        money(Math.abs(pnl), 0) +
        '</td>' +
        (mod.haircut
          ? '<td class="num ' + (proj < val - 1e-6 ? 'sell' : '') + '">' + money(proj, 0) + '</td>'
          : '') +
        '</tr>';
    });
    ...
    const head =
      ...
      (mod.haircut ? '<th class="num">If resolved now</th>' : '') +
      '</tr>';
    const foot =
      '<tr class="trow"><td>book</td><td></td><td class="num">' +
      fmt(tQ, 1) +
    ...
    const forced =
      st.forced > 1e-6
        ? '<p class="cap-forced">⚠ auto-deleverage force-closed ' +
          fmt(st.forced, 1) +
          ' CALL contracts from holders to reclaim capacity.</p>'
        : '';
    return (
      '<table class="cap-pos"><thead>' +
      head +
      '</thead><tbody>' +
      rows +
      foot +
  ...
  function drawBelief(st, mod) {
    ...
    plot(cv, (ctx, W, H) => {
      ...
      for (let i = 0; i <= 200; i++) {
        ...
        if (M.payoff(spec, x) > 1e-9) {
          if (!open) {
            ctx.moveTo(X(x), Y(0));
            open = true;
          }
          ctx.lineTo(X(x), Y(b.pdf(x)));
        } else if (open) {
          ctx.lineTo(X(x), Y(0));
          open = false;
        }
      ...
      const vline = (x, col, lab) => {
        ...
        ctx.fillText(lab, X(x), pad.t - 4);
      };
      const Kmark =
        spec.type === 'LINEAR'
          ? null
          : spec.type === 'SPREAD'
            ? (spec.lower + spec.upper) / 2
            : spec.type === 'GAUSSIAN'
              ? spec.center
              : spec.strike;
      vline(b.mu, buy, 'μ');
    ...
    plot(cv, (ctx, W, H) => {
      ...
      if (!mod.haircut) {
        ...
        if (!admitAt(hi)) {
          for (let i = 0; i < 40; i++) {
            const mid = (lo + hi) / 2;
            if (admitAt(mid)) lo = mid;
            else hi = mid;
          }
          wall = lo;
        }
      }
      const xMax = isFinite(wall)
        ? Math.max(wall * 1.25, 20)
        : Math.max(mmOf(st.book, key) * 0.8 + 50, 200);
      const N = 150;
      ...
      for (let i = 0; i <= N; i++) {
        const q = (i / N) * xMax;
        const Q = priceAt(q);
        const y = Q.admit || mod.haircut ? Q.exec : Number.NaN;
      ...
      ctx.font = '10px ui-monospace, monospace';
      ctx.textAlign = 'right';
      ctx.fillText(fmt(yMax, 1), pad.l - 4, pad.t + 6);
      ctx.fillText(fmt(f0, 1), pad.l - 4, Y(f0));
      ctx.textAlign = 'center';
      ctx.fillText('+contracts of ' + specLabel(spec) + ' →', (pad.l + W - pad.r) / 2, H - 5);
      ctx.strokeStyle = faint;
      ctx.setLineDash([2, 3]);
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.moveTo(pad.l, Y(f0));
      ctx.lineTo(W - pad.r, Y(f0));
      ctx.stroke();
      ctx.setLineDash([]);
      ctx.strokeStyle = mod.soft ? warn : accent;
      ctx.lineWidth = 2.2;
      ctx.lineJoin = 'round';
      ctx.beginPath();
      let started = false;
      for (const [q, y] of pts) {
        if (!isFinite(y)) break;
        const px = X(q),
          py = Y(y);
      ...
      if (isFinite(wall) && wall <= xMax) {
        ...
        ctx.setLineDash([5, 4]);
        ctx.beginPath();
        ctx.moveTo(X(wall), pad.t);
        ctx.lineTo(X(wall), H - pad.b);
        ctx.stroke();
        ctx.setLineDash([]);
        ctx.fillStyle = sell;
        ctx.textAlign = 'center';
        ctx.fillText('wall', X(wall), pad.t + 2);
        ctx.globalAlpha = 0.1;
        ctx.fillRect(X(wall), pad.t, W - pad.r - X(wall), H - pad.b - pad.t);
        ctx.globalAlpha = 1;
      } else if (mod.haircut) {
        ctx.fillStyle = warn;
        ctx.textAlign = 'right';
        ctx.fillText('no wall (haircut)', W - pad.r - 4, pad.t + 6);
      }
    });
  }
  ...
  function render() {
    ...
    if (fx.approx)
      infoHtml +=
        '<p class="cap-approx">⚠ Simplified model: a real hedge has basis &amp; counterparty risk; here it just scales the reserve by (1−h).</p>';
  ...
  function selectFix(id) {
    ...
    (FIXES[id].params || []).forEach((p) => {
      ...
      wrap.innerHTML =
        '<div class="row"><label>' +
        p.label +
        '</label><span class="val"></span></div><input type="range" min="' +
        p.min +
        '" max="' +
        p.max +
        '" step="' +
      ...
      const show = () => (val.textContent = p.fmt(Number.parseFloat(input.value)));
      show();
      input.addEventListener('input', () => {
        paramVals[id][p.key] = Number.parseFloat(input.value);
        show();
        applyFix();
      });
    });
  ...
  function applyFix() {
    ...
    STATE = replay(
      TAPE,
      cash0,
      curFix === 'none' || FIXES[curFix].explainOnly ? modifierFor('none') : modifierFor(curFix),
    );
    render();
  }
  function doBuy() {
    if (!STATE || FIXES[curFix].explainOnly) return;
    const size = clamp(Number.parseFloat($('#cap-buy-size', root).value) || 0, 0, 5000);
    if (size <= 0) return;
    const mod = activeMod();
    ...
    if (!r.fill || r.fill <= 1e-6) {
      ...
      out.textContent =
        'Rejected — ' +
        (mod.soft
          ? 'at the hard backstop'
          : 'no-buy band: this risk-opening buy needs cash ≥ ' + fmt(mod.margin, 2) + '×reserve.');
    } else {
  ...
  function readCash() {
    return clamp(Number.parseFloat($('#cap-cash', root).value) || 5000, 1000, 50000);
  }
  function rebuild() {
    builtCash = readCash();
    ...
    if (msg) {
      msg.textContent = 'Re-simulated to the gate with ' + money(builtCash) + ' liquidity.';
    }
  }
  ...
  function buildBuyControls() {
    ...
    if (seg) {
      ...
      [
        ...
        b.addEventListener('click', () => {
          buyType = v;
          $$('.cap-bt', seg).forEach((x) => x.classList.toggle('active', x.dataset.t === v));
          syncBuyParamVis();
          render();
        });
        seg.appendChild(b);
    ...
    const kIn = $('#cap-buy-k', root),
      wIn = $('#cap-buy-w', root);
    if (kIn) {
      kIn.value = buyK;
      kIn.addEventListener('input', () => {
        buyK = Number.parseFloat(kIn.value) || 100;
        render();
      });
    }
    if (wIn) {
      ...
      wIn.addEventListener('input', () => {
        buyW = Math.max(1, Number.parseFloat(wIn.value) || 8);
        render();
      });
    }
    syncBuyParamVis();
  ...
  function syncBuyParamVis() {
    const kWrap = $('#cap-buy-k-wrap', root),
      wWrap = $('#cap-buy-w-wrap', root);
  ...
  function mount() {
    ...
    noneBtn.className = 'cap-fix-btn seg active';
    noneBtn.dataset.fix = 'none';
    noneBtn.textContent = 'No fix';
    ...
    for (const id of FIX_ORDER) {
      ...
      b.className =
        'cap-fix-btn seg' +
        (FIXES[id].star ? ' star' : '') +
        (FIXES[id].explainOnly ? ' muted' : '');
      b.dataset.fix = id;
      b.innerHTML =
        '<span class="lt">' + id + '</span> ' + FIXES[id].name + (FIXES[id].star ? ' ★' : '');
    ...
    $('#cap-buy-size', root).addEventListener('keydown', (e) => {
      if (e.key === 'Enter') doBuy();
    });
    rebuild();
  }
  ...
  let mounted = false;
  const ensure = () => {
    if (!mounted && (!details || details.open)) {
      mounted = true;
      mount();
    }
  };
