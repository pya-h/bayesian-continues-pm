...
> Builds on **V1** (`TDD.md`, `TASKS.md`) and the spec (`MODEL.md`). V1 must be shipped and in use first. V2 adds the parts intentionally deferred from V1: richer beliefs, adaptive parameters, hedging, robust oracles/disputes, and horizontal scale. Where this doc disagrees with `MODEL.md`, this doc wins; deviations are called out.
>
...
Each workstream is independently shippable; **A (multi-modal beliefs)** is the headline feature. *(The original Workstreams **B** — leverage/margin/shorting/liquidation — and **H** — insurance fund — are now **V3**; see `docs/v3/TDD.md`. The original Workstream **F** — compliance / KYC tiers / geofencing — has been **removed from the project** entirely: this is play-money with no legal/identity layer. Workstream letters C–E, G are kept as-is to avoid churn against earlier references.)*
...
```
*(`audit_events` already exists from V1 — admin-action / top-up / lifecycle logging — and the V2-6 transaction ledger builds beside it; neither is a legal/compliance artifact.)*

*(Moved to V3: `positions.quantity` going negative, `margin_accounts`, `liquidations`, `insurance_fund`/`insurance_ledger`, `markets.cfg.margin_rates` — see `docs/v3/TDD.md §`Data Model.)*
...
- Ship order: **A (beliefs)** → **C, D, E** (adaptive, hedging, oracles) → **G (scale)** last. (See `V2-TASKS.md`.) Leverage/margin/liquidation + insurance are a separate **V3** track that builds on shipped V2.
