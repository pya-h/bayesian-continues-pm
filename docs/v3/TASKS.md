...
V3 is the home of the feature deferred out of V2: **leverage, margin, shorting, liquidation** — plus the **insurance fund** that backstops it. (See the "Moved to V3" banner in `docs/v2/TASKS.md`.)
...
- [ ] `services/insuranceSvc.ts`: `accrue(exec, amount, kind, ref)` written **inside the caller's `db.transaction()`** (same discipline as V2-6 `ledgerSvc`); `+= fee_pct × volume` wired into the trade path; invariant `balance == Σ ledger.amount`.
