...
(function (global) {
  ...
  function makeCapEngine(M) {
    ...
    function specLabel(s) {
      switch (s.type) {
        case 'LINEAR': return 'LINEAR';
        case 'SPREAD': return 'SPREAD ' + fmt0(s.lower) + '–' + fmt0(s.upper);
        case 'GAUSSIAN': return 'GAUSS ' + fmt0(s.center) + '±' + fmt0(s.width);
        case 'BINARY_CALL': return 'BIN-CALL ' + fmt0(s.strike);
        case 'BINARY_PUT': return 'BIN-PUT ' + fmt0(s.strike);
        case 'CALL': return 'CALL ' + fmt0(s.strike);
        case 'PUT': return 'PUT ' + fmt0(s.strike);
    ...
    const FIXES = {
      ...
      A: {
        ...
        params: [{ key: 'margin', label: 'open-margin m', min: 1.0, max: 1.2, step: 0.01, val: 1.05, fmt: (v) => v.toFixed(2) + '×' }],
      },
      B: {
      ...
      C: {
        ...
        params: [
          { key: 'kappa', label: 'strength κ', min: 0.05, max: 1.0, step: 0.05, val: 0.25, fmt: (v) => v.toFixed(2) },
          { key: 'a', label: 'convexity a', min: 1, max: 4, step: 0.5, val: 2, fmt: (v) => v.toFixed(1) },
        ],
      },
      D: {
      ...
      E: {
        ...
        params: [{ key: 'extraCash', label: 'fund top-up ΔC', min: 0, max: 12000, step: 500, val: 4000, fmt: (v) => '$' + Math.round(v).toLocaleString('en-US') }],
      },
      F: {
    ...
    function marginalAsk(spec, key, q, st, mod) {
      const shifted = { belief: st.belief, cash: st.cash, book: withMm(st.book, spec, key, mmOf(st.book, key) + q) };
      return quoteBuy(spec, key, 1, shifted, mod);
    }

    return {
      M, MU0, SIGMA0, CROWD_SPEC, CROWD_KEY, TAPE_STEP, TRADERS, GENESIS_BELIEF,
      cfg, round8, EPS, keyOf, specLabel, reserveBook,
      FIXES, FIX_ORDER, paramVals, modifierFor,
      backing, reserveMod, mmOf, withMm, congestionOf,
      quoteBuy, solveFill, genesis, getPos, stepBuy, buildTape, replay,
      gateInfo, marginalAsk,
    };
  }
